...
static std::mutex g_webScrollLockedMutex;
static std::unordered_set<int64_t> g_webScrollLockedSet;
...
{
    std::lock_guard<std::mutex> lock(g_webScrollLockedMutex);
    if (locked) {
        g_webScrollLockedSet.insert(webId);
        return;
    }
    g_webScrollLockedSet.erase(webId);
}
...
static bool IsWebScrollLocked(int64_t webId)
{
    std::lock_guard<std::mutex> lock(g_webScrollLockedMutex);
    return g_webScrollLockedSet.find(webId) != g_webScrollLockedSet.end();
}
...
- (BOOL)isPointInsideWeb:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    [AceWebResourcePlugin.getObjectMap enumerateKeysAndObjectsUsingBlock:^(
        ...
        int64_t webId = [key longLongValue];
        if (IsWebScrollLocked(webId)) {
            *stop = YES;
            return;
        }
...
- (void)setWindowFocusable:(BOOL)isFocusable {
    self.focusable = isFocusable;
    if (!isFocusable) {
        [self setIsFocused:NO];
        [self onViewFocusChanged:NO];
    }
}

- (void)onViewFocusChanged:(BOOL)gainFocus {
    _isFocused = gainFocus;
}
    ...
    const SyntheticPointerDataParams& params)
{
    ...
    pointerData.global_x = pointerData.display_x;
    pointerData.global_y = pointerData.display_y;
...
- (void)dispatchTouches:(NSSet *)touches {
    ...
    for (UITouch *touch in touches) {
        ...
        pointer_data.global_x = pointer_data.display_x;
        pointer_data.global_y = pointer_data.display_y;
        pointer_data.window_x = windowCoordinates.x * scale;
        pointer_data.window_y = windowCoordinates.y * scale;
