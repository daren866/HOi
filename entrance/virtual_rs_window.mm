...
static uint32_t GetWindowIdFromName(const std::string& windowName)
{
    auto pos = windowName.rfind(':');
    if (pos != std::string::npos) {
        std::string numStr = windowName.substr(pos + 1);
        if (!numStr.empty()) {
            const char* begin = numStr.c_str();
            char* end = nullptr;
            errno = 0;
            unsigned long val = std::strtoul(begin, &end, 10);
            if (end != begin && *end == '\0' && errno != ERANGE && val <= UINT32_MAX) {
                return static_cast<uint32_t>(val);
            }
            LOGW("Failed to parse windowId from windowName: %{public}s", windowName.c_str());
        }
    }
    return static_cast<uint32_t>([InstanceIdGenerator getAndIncrement]);
}
    ...
    std::shared_ptr<OHOS::AbilityRuntime::Platform::Context> context, void* windowView)
{
    ...
    uint32_t windowId = GetWindowIdFromName(windowName);
...
{
    std::string subWindowName = option->GetWindowName();
    if (CheckWindowNameExist(subWindowName)) {
        HILOG_ERROR("Window::CreateSubWindow : windowName exist! windowName=%{public}s", subWindowName.c_str());
        return nullptr;
    }

    uint32_t windowId = GetWindowIdFromName(subWindowName);
    ...
    window->SetWindowModal(option->IsWindowModal());
    window->SetWindowZLevel(option->GetWindowZLevel());
    window->SetRequestRect(option);
    windowView.zOrder = option->GetWindowZLevel();
    windowView.isModal = option->IsWindowModal();
...
WMError Window::ShowWindow() 
{
    ...
    if ([windowView_ showOnView:mainWindowView.superview]) {
        if (IsSubWindow()) {
            if (requestRect_.width_ <= 0 && requestRect_.height_ <= 0) {
                UIScreen *screen = [UIScreen mainScreen];
                CGFloat scale = screen.scale;
                requestRect_.width_ = screen.bounds.size.width * scale;
                requestRect_.height_ = screen.bounds.size.height * scale;
            }
            ResizeWindowTo(requestRect_.width_, requestRect_.height_);
            rect_ = requestRect_;
...
{   
    ...
    requestRect_.posX_ = x;
    requestRect_.posY_ = y;
    UIScreen *screen = [UIScreen mainScreen];
    CGFloat scale = screen.scale;
    if (scale == 0) {
        scale = 2;
    ...
    rect_.posX_ = requestRect_.posX_;
    rect_.posY_ = requestRect_.posY_;
    NotifySizeChange(rect_);
    return WMError::WM_OK;
}
...
WMError Window::SetFocusable(bool focusable)
{
    ...
    [windowView_ setWindowFocusable:focusable];
...
WMError Window::ResizeWindowTo(int32_t width, int32_t height) {
    ...
    requestRect_.width_ = width;
    requestRect_.height_ = height;
    UIScreen *screen = [UIScreen mainScreen];
    CGFloat scale = screen.scale;
    windowView_.frame = CGRectMake(windowView_.frame.origin.x, windowView_.frame.origin.y, width / scale, height / scale);

    rect_.width_ = requestRect_.width_;
    rect_.height_ = requestRect_.height_;
    NotifySizeChange(rect_);
    return WMError::WM_OK;
}
...
static void GetAvoidAreas(Window* window, std::map<AvoidAreaType, Rosen::AvoidArea>& avoidAreas) {
    for (AvoidAreaType type : { AvoidAreaType::TYPE_SYSTEM, AvoidAreaType::TYPE_CUTOUT,
                        AvoidAreaType::TYPE_NAVIGATION_INDICATOR, AvoidAreaType::TYPE_KEYBOARD }) {
        Rosen::AvoidArea area;
        window->GetAvoidAreaByType(type, area);
        avoidAreas[type] = area;
        if(type == AvoidAreaType::TYPE_SYSTEM || type == AvoidAreaType::TYPE_NAVIGATION_INDICATOR) {
            avoidAreas[type].bottomRect_ = emptyRect;
        }
    }
}
...
void Window::NotifySurfaceChanged(int32_t width, int32_t height, float density)
{
    ...
    if (!uiContent_) {
        ...
        std::map<AvoidAreaType, Rosen::AvoidArea> avoidAreas;
        GetAvoidAreas(this, avoidAreas);
        uiContent_->UpdateViewportConfig(config, WindowSizeChangeReason::RESIZE, avoidAreas);
    }
...
void Window::UpdateAvoidArea(const std::shared_ptr<Rosen::AvoidArea>& avoidArea, AvoidAreaType type)
{
    ...
    auto it = avoidAreas.find(type);
    if (it == avoidAreas.end()) {
        LOGE("AvoidArea type:%{public}d not found in avoidAreas", type);
        return;
    }
    
    const Rosen::AvoidArea& newArea = it->second;
    {
        std::lock_guard<std::recursive_mutex> lock(globalMutex_);
        auto iter = avoidAreaMap_.find(type);
        if (iter != avoidAreaMap_.end() && iter->second == newArea) {
            LOGD("UpdateAvoidArea type:%{public}d unchanged, skip", type);
            return;
        }
        avoidAreaMap_[type] = newArea;
    }
    std::shared_ptr<Rosen::AvoidArea> avoidAreaPtr = std::make_shared<Rosen::AvoidArea>(newArea);
    NotifyAvoidAreaChange(avoidAreaPtr, type);
...
void Window::SetWindowModal(bool isModal)
{
    isModal_ = isModal;
    if (windowView_) {
        windowView_.isModal = isModal;
    }
}

void Window::SetWindowZLevel(int32_t zLevel)
{
    zLevel_ = zLevel;
}
...
void Window::SetRequestRect(std::shared_ptr<WindowOption> option)
{
    requestRect_.width_ = option->GetWindowRect().width_;
    requestRect_.height_ = option->GetWindowRect().height_;
    requestRect_.posX_ = option->GetWindowRect().posX_;
    requestRect_.posY_ = option->GetWindowRect().posY_;
}
...
Orientation Window::GetRequestedOrientation()
{
    return orientation_;
}
...
{
    if (windowView_ == nullptr || !windowView_.isFocused) {
        LOGD("NotifyKeyboardHeightChanged: windowView is null or not focused");
        return;
    }
    uint32_t keyboardAvoidHeight = CalculateKeyboardAvoidHeight(height);
    if (keyboardAvoidHeight == lastNotifiedKeyboardHeight_) {
        LOGD("NotifyKeyboardHeightChanged: Keyboard height is the same as last notified");
        return;
    }
    lastNotifiedKeyboardHeight_ = keyboardAvoidHeight;
    keyBoardHieght_ = keyboardAvoidHeight;
    auto occupiedAreaChangeListeners = GetListeners<IOccupiedAreaChangeListener>();
    for (auto& listener : occupiedAreaChangeListeners) {
        if (listener != nullptr) {
            Rect rect = { 0, 0, 0, keyboardAvoidHeight };
...
uint32_t Window::CalculateKeyboardAvoidHeight(uint32_t keyboardHeight)
{
    if (windowView_ == nullptr || windowView_.window == nil) {
        LOGD("CalculateKeyboardAvoidHeight: windowView is null or window is nil");
        return 0;
    }

    if (keyboardHeight == 0) {
        // Keyboard dismissed: restore original position
        if (originWindowY_ > 0) {
            if (!isFullScreen_) {
                MoveWindowTo(rect_.posX_, originWindowY_);
            }
            originWindowY_ = 0;
        }
        return 0;
    ...
    UIScreen *screen = windowView_.window.screen;
    if (screen == nil) {
    ...
    CGRect windowRectInScreen = [windowView_ convertRect:windowView_.bounds toView:windowView_.window];
    CGFloat screenHeight = CGRectGetHeight(screen.bounds) * scale;
    CGFloat keyboardTop = screenHeight - static_cast<CGFloat>(keyboardHeight);
    int32_t currentWindowY = rect_.posY_;
    CGFloat windowHeight = CGRectGetHeight(windowRectInScreen) * scale;
    CGFloat windowBottom = CGRectGetMinY(windowRectInScreen) * scale + windowHeight;
    if (keyboardTop > windowBottom) {
        LOGD("CalculateKeyboardAvoidHeight: keyboard and window do not overlap");
        return 0;
    }

    CGFloat overlapHeight = std::max(0.0, windowBottom - keyboardTop);
    CGFloat maxMoveHeight = std::min(overlapHeight, static_cast<CGFloat>(currentWindowY));
    if (isFullScreen_) {
        return static_cast<uint32_t>(std::max(0.0, maxMoveHeight));
    }
    int32_t finalTargetY = std::max(0, currentWindowY - static_cast<int32_t>(maxMoveHeight));
    CGFloat reportHeight = std::max(0.0, overlapHeight - maxMoveHeight);
    if (finalTargetY != currentWindowY) {
        if (originWindowY_ == 0) {
            originWindowY_ = currentWindowY;
        }
        MoveWindowTo(rect_.posX_, finalTargetY);
    }
    return static_cast<uint32_t>(reportHeight);
}
...
WMError Window::SetLayoutFullScreen(bool status) {
    ...
    if (status == true) {
        ...
        SetMode(OHOS::Rosen::WindowMode::WINDOW_MODE_FULLSCREEN);
    } else {
        LOGI("isLayoutFullScreen is false");
        SetMode(OHOS::Rosen::WindowMode::WINDOW_MODE_UNDEFINED);
    }
